...
#def parse_vcd(vcd_file, start_time, end_time, csv):
def parse_vcd(vcd_file, start_time, end_time):
	...
	if startWrite:
		...
		for code in data.keys():
			...
			#print "size: %s, count: %s"%(size, count)
			if count>size:
				for i in write_file:
					if i.startswith('$var'):
						l = i.split()
						if l[4]==data[code]['nets'][0]['name'] or l[3]==code:
							idx = write_file.index(i)
							c1 = l[3]
							newCode = getNewCode(data)
							#data[newCode]={}
							l[3] = newCode
							write_file[idx] = '$var %s\t  %s %s\t%s  $end'%(l[1],l[2],l[3],l[4])
							newType = data[code]['nets'][0]['type']
							newName = data[code]['nets'][0]['name'] + '_v2 [31:0]'
							string = '$var %s\t  32 %s\t%s  $end'%(newType, c1, newName)
	...
	while True:
		...
		#print "testword:\n%s"%testword
		for code in data:
			#print data[code]['nets'][0]['name']
			if str(data[code]['nets'][0]['name']) == str(testword[2]) and str(data[code]['nets'][0]['hier'])==str(testword[1]):
				if int(data[code]['count'])!=int(testword[3]):
					print "%s not the same!"%code
	'''
